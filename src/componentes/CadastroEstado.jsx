...
export default class CadastroEstado extends React.Component {
   ...
   */
  getMsgExcluir() {
    return `Tem certeza que deseja excluir o Estado: "
            ${this.state.registro.estado}"?`;
  }
  ...
  async getRegistros(f, range) {
    ...
    const ret = await Api.getRegistrosPaginacaoCodNomeTab('estado', 'estado', 'cod_estado', filtro);
    if (!ret.status) {
  ...
  renderColunas() {
    return [
      <Column key={1} field="cod_estado"
                      style={{ width:60 + 'px' }} header="Código" />,
      <Column key={2} field="estado" 
                      style={{ width:200 + 'px' }} header="Estado" />,
      <Column key={3} field="sigla"
                      style={{ width:80 + 'px' }} header="Sigla" />,
          ...
          <div className="row col-sm-12">
            ...
            <div className="col-sm-2">
              <LabeledInput label="Código:"
                            disabled
                            value={this.state.registro.cod_estado}
                            onChange={this.onChange.bind(this, 'cod_estado')} />
            </div>
          
            <div className="col-sm-4">
              <LabeledInput label="Estado:"
                            value={this.state.registro.estado}
                            onChange={this.onChange.bind(this, 'estado')} />
            </div>
          
            <div className="col-sm-2">
              <LabeledInput label="Sigla:"
                            value={this.state.registro.sigla}
                            onChange={this.onChange.bind(this, 'sigla')} />
            </div>
          
            <div className="col-sm-3">
              <LabeledInput label="Código IBGE:"
  ...
  render() {
    return (
      <Cadastro tabela="tab_estado"
        pk="cod_estado"
        titulo="Cadastro de Estado"
        subTitulo="Configure os seus Estados!"
